# Thanks to https://mdneuzerling.com/post/r-on-aws-lambda-with-containers/ for most of this code
...
handle_event <- function(event) {
  ...
  if (http_req_element %in% names(event_content)) {
    ...
    if (is.null(event_content[[http_req_element]])) {
      ...
      event_content <- jsonlite::fromJSON(event_content[[http_req_element]]) # this assumes the body was JSON encoded
    }
  }
  
  # if we get (for example) an array of integers,
  # jsonlite will parse it into a numeric vector,
  # which won't work with do.call
  if (class(event_content) != "list") {
